...
namespace Electrical
{
    ...
    {
        /// <summary>The <see cref="Electrical.ElectricalComponent"> that connects to supply power 
        /// to this <see cref="Electrical.ElectricalComponent">.</summary>
        /// <remarks>All <see cref="Electrical.ElectricalComponent"> types must be able to receive 
        /// power from another <see cref="Electrical.ElectricalComponent">. However, some types 
        /// might have the option of multiple <see cref="Electrical.ElectricalComponent"> inputs, 
        /// and thus, might treat this field differently.</remarks>
        [SerializeField][Tooltip(Tooltips.inputNode)] protected ElectricalComponent inputNode;
        /// <summary>The <see cref="Electrical.ElectricalComponent"> that connects to receive power 
        /// from this <see cref="Electrical.ElectricalComponent">.</summary>
        /// <remarks>All <see cref="Electrical.ElectricalComponent"> types must be able to provide 
        /// power to another <see cref="Electrical.ElectricalComponent">. However, some types might 
        /// have the option of multiple <see cref="Electrical.ElectricalComponent"> outputs, and 
        /// thus, might treat this field differently.</remarks>
        [SerializeField][Tooltip(Tooltips.outputNode)] protected ElectricalComponent outputNode;
        //TODO:Position code
        protected Vector3 inputPosition;
        protected Vector3 outputPosition;
        /// <summary>The current ID number used to uniquely identify this 
        /// <see cref="Electrical.ElectricalComponent">.</summary>
        /// <remarks>The ID is allocated dynamically, at run time; it should never be directly 
        /// changed, and is only used for internal <see cref="Electrical.ElectricalComponent"> 
        /// comparison. It is serialized for the purpose of easy editor observation.</remarks>
        [SerializeField][Tooltip(Tooltips.id)] protected int id;
        // TODO: Set id to draw as read only.

        /// <summary>Internal count for current <see cref="Electrical.ElectricalComponent" 
        /// types that have been instantiated.</summary>
        ...
        /// <summary>Retrieves the total count of instantiated 
        /// <see cref="Electrical.ElectricalComponent"> types.</summary>
        /// <remarks>This property is derived from a value used to ensure unique identification 
        /// numbers; as such, it should not be used as a reliable count of currently active 
        /// <see cref="Electrical.ElectricalComponent"> types.</remarks>
        public int totalCount { get { return electricalComponentCount; } }
        /// <summary>Retrieves the current <see cref="Electrical.ElectricalComponent.id"> 
        ...
        /// <see cref="Electrical.ElectricalComponent">, in amps.</param>
        /// <param name="usingFixedVoltage">If <c>true</c>, this 
        /// <see cref="Electrical.ElectricalComponent"> will assume a fixed voltage, and apply 
        /// manipulation to the current. If <c>false</c>, this 
        /// <see cref="Electrical.ElectricalComponent"> will assume a fixed current, and apply 
        /// manipulation to the voltage.</params>
        /// <remarks>The default behaviour, on applied power, is to simply allow the power to move 
        /// directly into the <see cref="Electrical.ElectricalComponent.outputNode">; however, this 
        /// behaviour should be overridden for more advanced 
        /// <see cref="Electrical.ElectricalComponent"> types.</remarks>
        public virtual void ApplyPower(float voltage, float current, bool usingFixedVoltage)
        {
            if(outputNode != null)
            {
                // If the outputNode is not null, and thus connected; 
                // forward the power to the outputNode.
        ...
        {
            // Set the id for this ElectricalComponent to the next available id value, and 
            // increment electricalComponentCount to reflect an available id value for the 
            // next ElectricalComponent.
        ...
        /// <summary>Disconnects the <see cref="Electrical.ElectricalComponent.outputNode> from 
        /// this <see cref="Electrical.ElectricalComponent">, nullifying the reference.</summary>
        ...
        ///  <see cref="Electrical.ElectricalComponent">.</summary>
        /// <params name="inputNode">The <see cref="Electrical.ElectricalComponent"> to use as the 
        /// new input node.</params>
        ...
        ///  <see cref="Electrical.ElectricalComponent">.</summary>
        /// <params name="outputNode">The <see cref="Electrical.ElectricalComponent"> to use as the 
        /// new output node.</params>
        ...
        /// <summary>Tests this <see cref="Electrical.ElectricalComponent">, to see if it is 
        /// connected to a live circuit.</summary>
        /// <returns><c>True</c>, if this <see cref="Electrical.ElectricalComponent"> is connected 
        /// to a live circuit, else returns <c>false</c>.
        /// <remarks>This test will start reference hopping through the circuit via way of the 
        /// <see cref="Electrical.ElectricalComponent.outputNode"> reference. If this method loop 
        /// only reaches null references, this <see cref="Electrical.ElectricalComponent"> is not 
        /// in a live circuit; if the method reaches the same 
        /// <see cref="Electrical.ElectricalComponent">, identified by 
        /// <see cref="Electrical.ElectricalComponent.id">, we know that this 
        /// <see cref="Electrical.ElectricalComponent"> is in a live circuit.</remarks>
        public virtual bool TestLiveCircuit()
        {
            if(outputNode == null)
            {
                // If the outputNode reference is null, we are not connected to a live circuit; 
                // return false.
                return false;
            }
            else
            {
                // Else, we have a valid outputNode reference; start looping the TestLiveCircuit 
                // method, comparing against the local id.
                return outputNode.TestLiveCircuit(id);
            }
        }

        /// <summary>Tests this <see cref="Electrical.ElectricalComponent">, to see if it is 
        /// connected to a live circuit in relation to a prior 
        /// <see cref="Electrical.ElectricalComponent.inputNode">.</summary>
        /// <params name="sourceID">The <see creft=Electrical.ElectricalComponent.id"> value to 
        /// which we are comparing this <see cref="Electrical.ElectricalComponent"> against, in 
        /// order to determine if we have confirmed a complete circuit.</params>
        /// <returns><c>True</c>, if the referenced <see cref="Electrical.ElectricalComponent"> is 
        /// connected to a live circuit, else returns <c>false</c>.
        ...
        protected virtual bool TestLiveCircuit(int sourceID)
        {
            if(id == sourceID)
            {
                // If the local ID is the same as the source ID, we have arrived back at the 
                // original ElectricalComponent; confirming a live circuit. Return true.
                return true;
            }
            else if(outputNode == null)
            {
                // Else, if we have not arrived at the original ElectricalComponent, and the 
                // outputNode reference is null, we are have confirmed an incomplete circuit; 
                // return false.
                return false;
            }
            else
            {
                // Else, we can not confirm a complete or incomplete circuit; continue looping the 
                // test, using the next outputNode.
                return outputNode.TestLiveCircuit(sourceID);
            }
        }

        //TODO:TotalLength comment
    ...
    /// <summary>Holds base calculations for determining electrical values and 
    ...
    public static class Electrical
    {
        ...
        //TODO:CalculatePower(float, float) comments
        public static float CalculatePower(float voltage, float current)
        {
...
namespace Electrical.Utility
{
    ...
    {
        public const string inputNode = "The ElectricalComponent node supplying power into this " 
            + "ElectricalComponent.";
        public const string outputNode = "The ElectricalComponent node drawing power out of this " 
            + "ElectricalComponent.";
        public const string id = "The unique identification ID for this ElectricalComponent. Set " 
            + "dynamically; DO NOT EDIT";
    }

    // Colours for use in displaying custom editor GUI.
    public static class ElectricalColours
    {
        public static Color positiveSymbol = Color.red;
        public static Color negativeSymbol = Color.blue;
    }
